...
inline validate_camerainput(current_input) {
        /* Treat any compromised frame as invalid and ad hoc */
        if
        :: (current_input == COMPROMISED_IMAGE) ->
            satellite_state = fail;
            compromised_run = 0;
            printf("Validation: COMPROMISED image is ad hoc -> marking fail and dropping.\n");
            /* slide window */
...
ltl hack_blocked_with_validation { <> hack_state }
ltl safety_with_validation { [](!hack_state) }
